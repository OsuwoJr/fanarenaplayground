...
const EventCard: React.FC<EventCardProps> = ({ event, onParticipate }) => {
  ...
  // Mock user data for frontend experience
  const mockUser = 'MockUser'; // Placeholder until backend is connected

  const handleParticipate = (tournamentId: number) => {
    onParticipate(tournamentId, mockUser); // Using mock user for now
    ...
    <div className="grid-item">
      ...
      {isParticipateModalOpen && (
        <ParticipateTournamentModal 
          ...
          onParticipate={handleParticipate} // Uses mock data for now
        />
      )}
    </div>
  );
