...
interface CreateTeamModalProps {
  onClose: () => void;
  onCreate: (newTeam: Omit<Team, 'players'>) => void; // Adjusted newTeam type
}
...
interface Team {
  ...
  imageUrl: string;
  nftId: string;
  performance: number; // Include performance in this interface
...
const CreateTeamModal: React.FC<CreateTeamModalProps> = ({ onClose, onCreate }) => {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    const newTeam: Team = { name, sport, imageUrl, nftId, performance: 0, players: [] }; // Include performance
